...
│   ├── fn mathloop_data_dir()            # Resolve %APPDATA%/MathLoop
...
├── File Utilities
│   ├── fn copy_missing_resource_dir()    # Copy bundled assets, skip existing
...
│   └── fn copy_dir_missing()             # Recursive copy, skip existing files
└── Constants
    ├── DB_FILE = "mathloop.db"
...
bootstrap_mathloop_data(book_id?)
  -> run_migration()          # Check/catch-up migration version
  -> register_bundled_books() # Read public/books.json, add missing books
  -> resolve data_dir         # Legacy (top-level) or book-scoped (books/{bookId})
  -> create subdirectories
  -> copy_missing_resource_dir() for data, questions, answers, pages, question-fixes
  -> ensure_database()
  -> create_startup_backup()
  -> prune_backups(30)
  -> return BootstrapInfo { data_dir, db_path, ... }
```
